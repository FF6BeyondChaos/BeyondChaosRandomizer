...
TEST_SEED = "CE-4.2.1|normal|b c d e f g i j k m n o p q r s t u w y z makeover partyparty alasdraco capslockoff johnnydmad notawaiter remonsterate dancelessons electricboogaloo swdtechspeed mimetime|1679855259"
TEST_FILE = "FF3.smc"
...
def print_to_connection(output: str = ""):
    if gui_connection:
        gui_connection.send(output)
    else:
        print(output)
...
class WindowBlock():
    ...
    def read_data(self, rom_file_buffer: io.BytesIO):
...
def randomize(connection: Pipe = None, **kwargs) -> str:
    ...
    sleep(0.5)
    print_to_connection('You are using Beyond Chaos CE Randomizer version "%s".' % VERSION)
    if BETA:
        print_to_connection("WARNING: This version is a beta! Things may not work correctly.")
...
-   Use all flags EXCEPT the ones listed'''
    ...
    if len(data) % 0x400 == 0x200:
        print_to_connection("NOTICE: Headered ROM detected. Output file will have no header.")
        data = data[0x200:]
    ...
    infile_rom_buffer = io.BytesIO(open(infile_rom_path, "rb").read())
    outfile_rom_buffer = io.BytesIO(open(infile_rom_path, "rb").read())
    ...
    if not os.access(os.path.dirname(outfile_rom_path), os.W_OK):
        if gui_connection:
            ...
            raise PermissionError("The randomizer does not have write permissions to the given ROM output directory.")

    flags = flags.lower()
    activation_string = Options_.activate_from_string(flags)

    if version and version != VERSION:
        print_to_connection("WARNING! Version mismatch! "
    ...
    print_to_connection(activation_string)
        ...
        if type(random_boost_value) == bool:
            while True:
                ...
                try:
                    ...
                    print_to_connection("The supplied value for the randomness multiplier was not valid.")
    ...
    if Options_.is_flag_active("thescenarionottaken"):
        if Options_.is_flag_active("strangejourney"):
            print_to_connection("thescenarionottaken flag is incompatible with strangejourney")
        else:
    ...
    if Options_.is_flag_active('notawaiter') and not Options_.is_flag_active('ancientcave'):
        print_to_connection("Cutscenes are currently skipped up to Kefka @ Narshe")
        manage_skips()
    reseed()
    ...
    if Options_.is_flag_active('remonsterate'):
        outfile_backup = io.BytesIO(outfile_rom_buffer.getbuffer().tobytes())
                    ...
                    try:
                        ...
                        if isinstance(response, str):
                            ...
                            if not isinstance(response, OverflowError) and not \
                                ...
                                print("Response received. No parent process.")
                                raise response
                    except EOFError:
                        break

            except (OverflowError, ReferenceError) as e:
                print_to_connection("Remonsterate: An error occurred attempting to remonsterate. Trying again...")
                # Replace backup file
        ...
        if Options_.is_flag_active('expboost') and type(exp_boost_value) == bool:
            while True:
                try:
                    ...
                    print_to_connection("The supplied value for the EXP multiplier was not a positive number.")
        for m in monsters:
        ...
        if type(gp_boost_value) == bool:
            while True:
                try:
                    ...
                    print_to_connection("The supplied value for the gp multiplier was not a positive number.")
        for m in monsters:
        ...
        if type(swdtech_speed) == bool:
            while True:
                ...
                try:
                    ...
                    print_to_connection("The supplied speed was not a valid option. Please try again.")
        change_swdtech_speed(outfile_rom_buffer, random, swdtech_speed)
    if Options_.is_flag_active("cursepower"):
        change_cursed_shield_battles(outfile_rom_buffer, random, Options_.get_flag_value("cursepower"))
    ...
    print_to_connection("Randomization successful. Output filename: %s\n" % outfile_rom_path)
    ...
    try:
        ...
        for argument in args[1:]:
            if 'source=' in argument:
                ...
                print('Keyword unrecognized or missing: ' + str(
