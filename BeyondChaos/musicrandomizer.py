...
def put_somewhere(romdata, newdata, desc, f_silent=False):
    ...
    if not success:
        if not f_silent:
            print("ERROR: not enough free space to insert {}\n\n".format(desc))
...
def process_custom_music(data_in, eventmodes="", opera=None, f_randomize=True, f_battleprog=True, f_mchaos=False,
                         f_altsonglist=False):
    global freespace
    data = data_in
    ...
    try:
        songcountloc = int(CONFIG.get('MusicPtr', 'songcount'), 16)
    except ValueError:
        songcountloc = to_default('songcount')
    ...
    for ident, s in songtable.items():
        spooler("{} pool ({}/{}): {}".format(ident, len([i for i in s.choices if i == native_prefix + ident]),
                                             len(s.choices), s.choices))
    ...
    def process_battleprog():
        ...
        def intensity_subset(imin=0, gmin=0, imax=99, gmax=99):
            subset = {k: v for k, v in intensitytable.items() if
                      v[0] >= imin and v[0] <= imax and v[1] >= gmin and v[1] <= gmax and usage_id(k) not in used_songs}
            return subset
        ...
        if old_method:
            ...
            while retry:
                retry = False
                battlechoices = random.sample(
                    [(k, sum(intensitytable[k]), intensitytable[k][0]) for k in intensitytable.keys()], battlecount)
            ...
            while True:
                try:
                    ...
                    boss, (bi, bg) = random.choice(
                        list(intensity_subset(imin=bt, gmin=max(boss1_g_min, eg), gmax=sg).items()))
    ...
    while keeptrying and attempts <= 1000:
        ...
        for ident, s in songtable.items():
            ...
            elif isfile(os.path.join(MUSIC_PATH, s.changeto + ".mml")) or isfile(
                    os.path.join(MUSIC_PATH, usage_id(s.changeto) + ".mml")) or isfile(
                    os.path.join(MUSIC_PATH, vbase + vid + ".mml")):
                        ...
                        if file_to_check in potential_files:
                            ...
                            try:
                                with open(os.path.join(MUSIC_PATH, file_to_check + ".mml"), 'r') as mmlf:
                                    mml = mmlf.read()
                                varlist = get_variant_list(mml, sfxmode=True if s.id == 0x29 or s.id == 0x4F or (
                                            s.id == 0x5D and windy_intro) else False)
        ...
        for ident, s in songtable.items():
            if not s.is_pointer:
                try:
                    data, start, end = put_somewhere(data, s.data, "  (song) [{:02x}] {}".format(s.id, s.changeto),
                                                     True)
    ...
    for i, (id, s) in enumerate(sorted(changed_songs.items())):
        ...
        if s[1] in metadata:
            ...
            spoiltext[i] += "\n" + " " * 8 + "Composed by {} -- Arranged by {}".format(metadata[s[1]].composer,
                                                                                       metadata[s[1]].arranged)
    ...
    def adjust_event(dat, offset, oldid, newid):
        ...
        while True:
            ...
            if op == 0xFE:
                ...
                loc += 1
                while dat[loc] != 0xFE and dat[loc + 2] <= 2:
                    loc += 3
