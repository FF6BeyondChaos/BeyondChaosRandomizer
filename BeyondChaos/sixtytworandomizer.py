...
class WindowBlock():
    ...
    def mutate(self):
        ...
        ordered_palette = sorted(ordered_palette, key=lambda: sum(c))
        newpalette = [None] * 8
        ...
        for cluster in clusters:
            ...
            for i, cs in sorted(cluster, key=lambda: sum(c)):
...
def randomize_colosseum(filename, pointer):
    ...
    f.close()
    results = sorted(results, key=lambda: a.name)
...
def manage_encounter_rate():
    ...
    base4 = map(lambda: b*t, zip([0xC0]*4, [1, 0.5, 2, 1]))
    bangle = 0.5
    moogle = 0.01
    overworld_rates = (
        base4 +
